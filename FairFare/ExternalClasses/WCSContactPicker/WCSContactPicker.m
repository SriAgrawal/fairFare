...
@interface WCSContactPicker ()<UISearchBarDelegate, UISearchDisplayDelegate> {
    CNContactStore * _contactStore;
    NSArray *arr, *finalArray;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ( _contacts.count )
    {
       static NSString *CellIdentifier = @"ContactCell";
        ...
        if (self.searchBarActive) {
            if (finalArray.count) {
                Contact *contact = [self personFromContact:(CNContact*)finalArray[indexPath.row]];
                cell.displayName = [contact.displayName length] ? contact.displayName :@"";
                cell.phoneNumber = [[contact.phones firstObject] length] ? [NSString stringWithFormat:@"%@",[contact.phones firstObject]]: @"";
                cell.emailAddress = [[contact.emails firstObject] length] ? [NSString stringWithFormat:@"%@",[contact.emails firstObject]] : @"";
            }else {
                Contact *contact = [self personFromContact:(CNContact*)arr[indexPath.row]];
                cell.displayName = [contact.displayName length] ? contact.displayName :@"";
                cell.phoneNumber = [[contact.phones firstObject] length] ? [NSString stringWithFormat:@"%@",[contact.phones firstObject]]: @"";
                cell.emailAddress = [[contact.emails firstObject] length] ? [NSString stringWithFormat:@"%@",[contact.emails firstObject]] : @"";
            }
        }else{
            CNContact *obj = [_contacts objectAtIndex:indexPath.row];
            cell.displayName = obj.givenName;
        }
        return cell;
    }
    else
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    UIView *headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width - 10, 35)];
    headerView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:199.0/255.0 blue:204.0/255.0 alpha:1.0f];
    contactSearchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(headerView.frame.origin.x + 5,headerView.frame.origin.y + 3,headerView.frame.size.width - 10,headerView.frame.size.height - 5)];
   
    
    contactSearchBar.placeholder = NSLocalizedString(@"Search for Contacts",nil);
    contactSearchBar.delegate = self;
    [headerView addSubview:contactSearchBar];
    return headerView;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ( _contacts.count )
    {
        if ( [self.delegate respondsToSelector:@selector(picker:didSelectContact:)] ) {
            Contact * contacts;
            if (self.searchBarActive) {
            contacts = [self personFromContact:(CNContact*)arr[indexPath.row]];

            }else {
                contacts = [self personFromContact:(CNContact*)_contacts[indexPath.row]];
            }

            if ([contacts.phones count] > 0) {
                if(self.searchBarActive) {
                    [self.delegate picker:self didSelectContact:[self personFromContact:(CNContact*)arr[indexPath.row]]];
                }
                else {
                ...
                [[AlertView sharedManager] presentAlertWithTitle:@"" message:@"Number does not exist" andButtonsWithTitle:@[@"Ok"] onController:self dismissedWith:^(NSInteger index, NSString *buttonTitle) {
                }];
...
- (void)filterContentForSearchText:(NSString*)searchText scope:(NSString*)scope{
    ...
    arr  = [_contacts filteredArrayUsingPredicate:resultPredicate];
}
...
//                                                         selectedScopeButtonIndex]]];
        ...
        if ([contactSearchBar.text  isEqual: @""]) {
            finalArray = _contacts;
           // [finalArray arrayByAddingObjectsFromArray:_contacts];
            [self.tableView reloadData];
        }
